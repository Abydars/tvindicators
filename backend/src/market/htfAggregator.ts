...
export class HtfAggregator extends EventEmitter {
  private candles: Candle[] = [];
  private currentCandle: Candle | null = null;
  private currentHTFTime: number = 0;

  constructor() {
    super();
  }

  public async initialize(m1Candles: Candle[]): Promise<void> {
    try {
      const history = await OandaClient.getCandles(undefined, 'M15', 200);
  ...
  public handleM1Close(m1Candle: Candle): void {
    ...
    if (htfOpenTime === this.currentHTFTime) {
      ...
      if (m1Candle.high > this.currentCandle.high) this.currentCandle.high = m1Candle.high;
      if (m1Candle.low < this.currentCandle.low) this.currentCandle.low = m1Candle.low;
  ...
  public injectClosedHTFCandle(candle: Candle): void {
  ...
  private aggregateFromM1History(m1Candles: Candle[]): void {
    const buckets: { [key: number]: Candle[] } = {};
    for (const c of m1Candles) {
      const htfOpenTime = Math.floor(c.time / (15 * 60 * 1000)) * (15 * 60 * 1000);
      if (!buckets[htfOpenTime]) buckets[htfOpenTime] = [];
...
export const htfAggregator = new HtfAggregator();
export default htfAggregator;
