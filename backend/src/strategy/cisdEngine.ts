import { Candle, CisdPotential } from '../../../shared/types';

export interface CisdOutput {
  signal: 0 | 1 | 2; // 0 = none, 1 = bearish, 2 = bullish
  originLevel: number | null;
  originIndex: number | null;
}

export class CisdEngine {
  private bearishPotentialLevels: CisdPotential[] = [];
  private bullishPotentialLevels: CisdPotential[] = [];
   ...
   */
  public evaluate(candles: Candle[], cisdTolerance: number): CisdOutput {
    if (candles.length < 3) {
    ...
    const getCandle = (offset: number): Candle => {
    ...
    let signal: 0 | 1 | 2 = 0;
    let originLevel: number | null = null;
